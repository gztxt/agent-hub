...
PROFILES: List[dict] = [
    ...
    {"id": "claude", "name": "Claude Code", "kind": "agent",
     ...
     "chat": {"adapter": "claude"},
     "desc": "Anthropic 编码 Agent（CLI/TUI），经 CCR 网关对话"},
    {"id": "pi", "name": "Pi Agent", "kind": "agent",
     "detect": {"proc": [r"next-server", r"(^|/)pi( |$)"]},
    ...
    {"id": "ccr", "name": "CCR Gateway", "kind": "gateway",
     ...
     "cli": None, "port": 3456, "ui": None, "panel": None,
     "desc": "Claude↔多模型路由网关 :3456/3457（生命线，只观测）"},
    {"id": "fcc", "name": "FCC Gateway", "kind": "gateway",
     "detect": {"proc": [r"fcc-server"], "systemd": ["fcc"]},
     "cli": None, "port": 8082, "ui": None, "panel": "http://127.0.0.1:18083/admin",
    ...
    {"id": "ccpocket", "name": "CCPocket Bridge", "kind": "gateway",
     ...
     "cli": None, "port": 8765, "ui": None, "panel": "http://127.0.0.1:8765",
     "desc": "手机桥接 WebSocket :8765"},
    ...
    {"id": "gotty", "name": "GoTTY Terminal", "kind": "tool",
     "detect": {"proc": [r"gotty"]},
     "cli": None, "port": 12700, "ui": None, "panel": "http://127.0.0.1:12700",
    ...
    {"id": "ollama", "name": "Ollama", "kind": "tool",
     ...
     "cli": None, "port": 11434, "ui": None, "panel": "http://127.0.0.1:11434",
     "desc": "本地模型运行时 :11434"},
    {"id": "chromium", "name": "Chromium (容器)", "kind": "tool",
     "detect": {"docker": ["chromium"]},
     "cli": None, "port": 3000, "ui": None, "panel": "http://127.0.0.1:3000",
     "desc": "无头浏览器服务 :3000"},
...
def entries_for(p: dict, status: str) -> List[dict]:
    """卡片按钮 = 画像驱动（②服务/工具仅快捷方式；Agent 才有 embed/term/chat）"""
    es = []
    kind = p["kind"]
    if kind == "agent":
        if p.get("ui"):
            es.append({"type": "embed", "label": "嵌入会话", "url": p["ui"]})
            es.append({"type": "open", "label": "新窗口", "url": p["ui"]})
