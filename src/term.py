...
async def list_sessions():
    _reap()
    return {"sessions": [s.to_dict() for s in _sessions.values()]}
...
def _reap():
    for s in list(_sessions.values()):
        if not s.alive and time.time() - s.last_io > 60:
            try:
                os.close(s.fd)
            except OSError:
                pass
            ...
            continue
        if s.alive and time.time() - s.last_io > IDLE_TTL_S:
            s.kill()
