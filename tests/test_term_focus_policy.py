...
所以这里守的是"**位置**"，不是"存在"：
  ...
  2. 六个 termConnect 入口里，恰好 4 个带 `{ user: true }`（用户主动），
...
class TestFocusPolicy(unittest.TestCase):
    ...
    def test_user_entry_points_count(self):
        """4 个用户主动入口带 user:true；2 个自动路径不带。钉数量也钉身份。"""
        # 包装器 termOpenChip 的定义行本身也是 `termConnect(sid, agent, { user: true })`：
        # 它是字面上的第 4 处匹配，但不是调用点。第一版就在这条上红，别把测试 bug 记成产品 bug。
        user = [l for _, l in _lines(r"termConnect\([^)]*\{ user: true \}\)") if "function " not in l]
        self.assertEqual(len(user), 3, f"直接调用点应恰好 3 处带 user:true，实得 {len(user)}")
